...
class ListStandardsBySubDomainHandler implements DBHandler {

  private static final Logger LOGGER = LoggerFactory.getLogger(ListStandardsBySubDomainHandler.class);
  private final ProcessorContext context;

  public ListStandardsBySubDomainHandler(ProcessorContext context) {
  ...
  public ExecutionResult<MessageResponse> checkSanity() {
    ...
    if (context.domainId() == null) {
      LOGGER.error("checkSanity() failed. Sub Domain Id is null!");
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse(), ExecutionResult.ExecutionStatus.FAILED);
    } else if (context.domainId().isEmpty()) {
      LOGGER.error("checkSanity() failed. sub Domain Id is empty!");
      return new ExecutionResult<>(MessageResponseFactory.createNotFoundResponse("The queried subdomain Id is not found in the DB."), ExecutionResult.ExecutionStatus.FAILED);
    }
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    if (!AJEntityDefaultSubDomain.exists(Long.parseLong(context.domainId()))) {
      LOGGER.debug("validateRequest() failed");
      return new ExecutionResult<>(MessageResponseFactory.createNotFoundResponse("The queried sub domain Id is not found in the DB."), ExecutionResult.ExecutionStatus.FAILED);
    }
    ...
    LazyList<AJEntityDefaultSubDomain> subdomain = AJEntityDefaultSubDomain.findBySQL(AJEntityDefaultSubDomain.FETCHBYSUBDOMAINANDCOURSE_QUERY, Long.parseLong(context.domainId()), Long.parseLong(context.courseId()));
    if (subdomain.size() <= 0) {
      LOGGER.debug("found association");
      return new ExecutionResult<>(MessageResponseFactory.createInvalidRequestResponse("Course and Subdomain association not found"), ExecutionResult.ExecutionStatus.FAILED);
    } 
